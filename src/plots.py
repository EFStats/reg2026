...
def makeplots(df: pd.core.frame.DataFrame,
    ...
    ax.set_xlim([datetime.date(2026, 2, 1),
                 datetime.date(2026, 6, 1)]) # target: 3rd Sept (2026, 8, 19)
...
f'''{total} total regs, out of which {partial + paid} paid at least partially.'''
    ...
    # Lower-left plot
    annot    = \
f'''{nb_normal+nb_contributor+nb_spons+nb_super} total regs ({nb_normal} normal, {nb_contributor} contributors, {nb_spons} sponsors, {nb_super} supersponsors).'''
